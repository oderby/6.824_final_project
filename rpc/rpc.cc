		...
		char *b, int sz)
{
        VERIFY(clt_nonce>0);
	ScopedLock rwl(&reply_window_m_);
        // find the point in the list we should insert this reply at (list is
        // ordered by xid).
        std::list<reply_t>::iterator it = reply_window_[clt_nonce].begin();
        while (it!=reply_window_[clt_nonce].end() && it->xid!=xid) {
          it++;
        }
        VERIFY(it->xid == xid); //at this point, it should point to the first
                                 //element in the list which has a newer xid.
        // create the reply structure to insert
        (*it).buf = b;
        (*it).sz = sz;
        (*it).cb_present = true;
